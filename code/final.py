...
if (sys.argv[1] == "optimization"):
    ...
    ngram_range = (2,3)     #to consider unigrams, bigrams and trigrams, these are needed to consider the order of the instructions
    ...
    #Each term found by the analyzer during the fit is assigned a unique integer index corresponding to a column in the resulting matrix
    """
    vectorizer = CountVectorizer(lowercase=False,           #no needs because all the words are alredy lowercase
    ...
    vectorizer = TfidfVectorizer(lowercase=False,           #no needs because all the words are alredy lowercase
                            ngram_range=ngram_range,
                            max_df=max_df,
                            min_df=min_df
                            )
    ...
    model = MultinomialNB(alpha=0.1, class_prior=[0.1, 0.9], fit_prior=True)
...
if (sys.argv[1] == "compiler"):
    ...
    #Each term found by the analyzer during the fit is assigned a unique integer index corresponding to a column in the resulting matrix
    ngram_range = (1,3)     #to consider unigrams, bigrams and trigrams, these are needed to consider the order of the instructions
    min_df = 5             #ignore terms that have a document frequency strictly lower than the given threshold
    max_df = 1.             #ignore terms that have a document frequency strictly higher than the given threshold
    #max_features = 10000
